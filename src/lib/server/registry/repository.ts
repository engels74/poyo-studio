...
import { VIDEO_AUDIT_RECORDS, VIDEO_REGISTRY } from '../../features/registry/video-registry';
...
export function seedVideoRegistry(database: Database): void {
  database.transaction(() => {
    database
      .query(
        `INSERT INTO registry_versions(version,source_hash,manifest_hash,verified_at,status) VALUES (?,?,?,?,'current') ON CONFLICT(version) DO UPDATE SET source_hash=excluded.source_hash,manifest_hash=excluded.manifest_hash,verified_at=excluded.verified_at,status='current'`
    ...
    for (const entry of VIDEO_REGISTRY.entries)
      insert.run(
        VIDEO_REGISTRY.version,
        entry.key,
        entry.publicModelId,
    ...
    for (const record of VIDEO_AUDIT_RECORDS)
      insert.run(
        VIDEO_REGISTRY.version,
        record.key,
        record.publicModelIds.join(','),
        ...
        record.status,
        JSON.stringify(record),
        JSON.stringify({ sourceUrl: record.sourceUrl, verifiedAt: VIDEO_REGISTRY.verifiedAt }),
        JSON.stringify([record.reason])
      );
  })();
}
