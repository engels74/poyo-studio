...
export function videoCatalogue(query = '') {
  const needle = query.trim().toLowerCase();
  return VIDEO_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.status === 'current' &&
      (!needle ||
        [entry.displayName, entry.provider, entry.publicModelId, entry.workflow].some((value) =>
...
export function modelCatalogue(query = '') {
  return [
    ...imageCatalogue(query).map((entry) => ({ ...entry, modality: 'image' as const })),
    ...videoCatalogue(query)
  ];
}
