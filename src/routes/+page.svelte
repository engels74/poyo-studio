...
<div class="route-shell">
  ...
  <section aria-labelledby="recent-heading" class="mt-8 border-t border-border pt-5">
    <div class="flex items-center justify-between gap-4"><div><p class="eyebrow-label">Library</p><h2 id="recent-heading" class="mt-1 text-base font-semibold">Recent generations</h2></div><a class="focus-ring rounded text-xs font-semibold text-primary hover:underline" href="/library">Open library</a></div>
    {#if data.dashboard.recent.length}<div class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{#each data.dashboard.recent as group}<a href={`/library/${group.jobId}`} class="focus-ring overflow-hidden rounded-lg border border-border bg-card no-underline"><MediaPreview mediaKind={group.representative?.mediaKind ?? group.modality} src={group.representative?.mediaUrl ?? null} alt={`Preview for ${group.displayName}`} class="aspect-video" /><div class="p-3"><p class="truncate text-sm font-semibold">{group.displayName}</p><p class="mt-1 truncate text-xs text-muted-foreground">{group.promptExcerpt ?? group.workflow}</p></div></a>{/each}</div>
