...
function snapshot(tiers = supported.map(tier), expiresAt = '2026-07-21T00:00:00.000Z') {
  return {
    ...
    registryVersions: { image: 'image-2026-08-24.1', video: 'video-2026-08-26.1' },
    source: {
      kind: 'published',
...
describe('browser-safe published pricing estimates', () => {
  ...
  test('estimates current WAN image-to-video and frame-to-video entries without pricing retired keys', () => {
    expect(
      ...
      credits: 60,
      signature:
        'version=pricing-signature-v1|registry=video-2026-08-26.1|model=wan2.7-image-to-video|workflow=image-to-video|unit=per-second|duration=5|resolution=720p',
      availability: 'available'
    });
    expect(
      estimateNormalizedRegistryRequest({
